...
function SingUp() {
  ...
  return (
      ...
      <form className='create_new_account' onSubmit={handleSubmit(submitRegisterUser)}>
        ...
        <span className='signin_span'>Already have an account?<Link to="/signin" className="signin_link">Sign In</Link></span>
        <span className='error_registration'>{registrationNewUserError ? "Регистрация не удалась" : null}</span>
        <span className='error_registration'>{registrationUserInfo ? `${registrationUserInfo.username} Вы успешно зарегистрировались` : null}</span>
